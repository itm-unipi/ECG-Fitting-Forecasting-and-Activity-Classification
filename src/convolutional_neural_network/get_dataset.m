function [dataset, targets] = get_dataset(resources_path, window_size)
    ...
    for k = 1 : length(csv_timeseries)
        ...
        while true
            ...
            % Extract the new window and compute the ecg mean
            current_window = table2array(raw_timeseries(start_window_index : end_window_index, :))';
            current_target = mean(table2array(raw_targets(start_window_index : end_window_index, 1)));
            dataset = [ dataset; { current_window } ];
            targets = [ targets; current_target ];

            fprintf("file: %s, window: %d, mean: %d\n", csv_timeseries(k).name, i, current_target);
